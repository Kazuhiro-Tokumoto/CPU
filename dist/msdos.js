...
class FAT12Disk {
    ...
    flush() {
        for (const n of this._dirty) {
            ...
            let bin = '';
            for (let i = 0; i < 512; i++)
                bin += String.fromCharCode(sec[i]);
            localStorage.setItem(`msdos_s${n}`, btoa(bin));
        }
        this._dirty.clear();
    }
    wipeStorage() {
        for (let n = 0; n < this.TOTAL_SECTORS; n++)
            localStorage.removeItem(`msdos_s${n}`);
        this._cache = {};
        this._dirty.clear();
    }
    ...
    format(label = 'MYDOS      ') {
        ...
        for (let i = 0; i < 11; i++)
            this.w8(0, 43 + i, lb.charCodeAt(i));
        const fs = 'FAT12   ';
        for (let i = 0; i < 8; i++)
            this.w8(0, 54 + i, fs.charCodeAt(i));
        this.w8(0, 510, 0x55);
        this.w8(0, 511, 0xAA);
        this._dirty.add(0);
        // FAT
        for (const fs of [this.FAT1_START, this.FAT2_START]) {
            const s = this._sec(fs);
            s.fill(0);
            s[0] = 0xF0;
            s[1] = 0xFF;
            s[2] = 0xFF;
    ...
    _rootSectors() {
        const a = [];
        for (let i = 0; i < this.ROOT_SECTORS; i++)
            a.push(this.ROOT_START + i);
        return a;
    }
    _chainSectors(c) {
        const a = [];
    ...
    _writeName83(sec, off, name) {
        ...
        for (let i = 0; i < 8; i++)
            this.w8(sec, off + i, i < n.length ? n.charCodeAt(i) : 0x20);
        for (let i = 0; i < 3; i++)
            this.w8(sec, off + 8 + i, i < e.length ? e.charCodeAt(i) : 0x20);
    }
    _allocEntry(sectors) {
        for (const sec of sectors) {
            for (let slot = 0; slot < 16; slot++) {
    ...
    readFile(parts, name) {
        ...
        if (!f)
            return null;
        const buf = [];
        let c = f.cluster, rem = f.size;
        while (c >= 2 && c < 0xFF8) {
            const sec = this.c2s(c);
            const take = Math.min(512, rem);
            for (let i = 0; i < take; i++)
                buf.push(this.r8(sec, i));
            rem -= take;
            c = this.getFAT(c);
        }
    ...
    mkdir(parts, name) {
        ...
        // . entry
        for (let i = 0; i < 11; i++)
            this.w8(sec, i, '.          '.charCodeAt(i));
        this.w8(sec, 11, 0x10);
        this.w16(sec, 26, c);
        // .. entry
        for (let i = 0; i < 11; i++)
            this.w8(sec, 32 + i, '..         '.charCodeAt(i));
        this.w8(sec, 43, 0x10);
        this.w16(sec, 58, 0);
        this._dirty.add(sec);
...
class DOSShell {
    ...
    execute(line) {
        ...
        if (redirOut || pipeMore) {
            this.term.print = oPrint;
            this.term.println = oPrintln;
            const txt = captured.join('');
            if (redirOut) {
                const enc = new TextEncoder().encode(txt);
                const [dp, fn] = this._splitPath(redirOut);
                try {
                    this.disk.writeFile([...this.cwdParts, ...dp], fn, enc);
                    this.disk.flush();
                }
                catch (e) {
                    this.term.println('Write error: ' + e.message);
                }
            }
            if (pipeMore)
    ...
    _dispatch(name, args) {
        switch (name) {
            case 'CLS':
                ...
                break;
            case 'VER':
                this.term.println('MYDOS Version 1.0  Copyright (C) 2025');
                break;
            case 'DIR':
                this._dir(args);
                break;
            ...
            case 'PATH':
                ...
                break;
            case 'PROMPT':
                this._promptCmd(args);
                break;
            case 'DATE':
                this.term.println('現在の日付: ' + new Date().toLocaleDateString());
                break;
            case 'TIME':
                this.term.println('現在の時刻: ' + new Date().toLocaleTimeString());
                break;
            case 'VOL':
                this.term.println(' ドライブ A のボリュームラベルは MYDOS\n ボリュームシリアル番号は DEAD-BEEF');
                break;
            case 'FORMAT':
                this._format(args);
                break;
            ...
            case 'MEM':
                ...
                break;
            case 'LABEL':
                this.term.println('[LABEL] 未サポート');
                break;
            case 'PRINT':
                this.term.println('[PRINT] プリンタ未サポート');
                break;
            case 'DEBUG':
                this.term.println('[DEBUG] 未実装');
                break;
            case 'EDIT':
                this.term.println('[EDIT] index.htmlのエディタをご利用ください');
                break;
            case 'PAUSE':
                this.term.println('続行するには何かキーを押してください...');
                break;
            case 'REM': break;
            case 'BREAK':
                this.term.println('^C');
                break;
            case 'VERIFY':
                this.term.println('VERIFY is OFF');
                break;
            case 'DISKCOMP':
            case 'DISKCOPY':
                this.term.println('このコマンドはサポートされていません');
                break;
            case 'CALL':
                this.term.println('CALL: ' + args.join(' '));
                break;
            case 'GOTO':
                this.term.println('GOTOはバッチファイル専用です');
                break;
            case 'IF':
                this._if(args);
                break;
            case 'FOR':
                ...
                break;
            case 'EXIT':
                this.term.println('シェルを終了できません（ここはDOSです）');
                break;
            case 'HELP':
                this._help(args);
                break;
            default:
                this._runFile(name, args);
                break;
        }
    }
    _dir(args) {
        ...
        if (entries === null) {
            this.term.println('指定されたパスが見つかりません');
            return;
        }
        if (!bare) {
            this.term.println(` ドライブ A のボリュームラベルは MYDOS`);
        ...
        if (wide) {
            ...
            if (!bare && entries.length === 0)
                this.term.println('ファイルがありません');
            for (const e of entries) {
                if (e.attr & 0x10) {
                    dc++;
                    if (!bare)
                        this.term.println(`${e.name.padEnd(20)}<DIR>`);
                    else
                    ...
                    tb += e.size;
                    if (!bare)
                        this.term.println(`${e.name.padEnd(20)}${String(e.size).padStart(10)} バイト`);
                    else
                        this.term.println(e.name);
                }
            }
    ...
    _cd(args) {
        ...
        for (const p of rel.split('/').filter(Boolean)) {
            ...
            if (p === '..') {
                ...
                newP.push(p.toUpperCase());
        }
        if (!this.disk._resolveDirSectors(newP)) {
            this.term.println('指定されたパスが見つかりません');
            return;
        }
        this.cwdParts = newP;
    ...
    _md(args) {
        if (!args[0]) {
            this.term.println('ディレクトリ名を指定してください');
            return;
        }
        const [dp, fn] = this._splitPath(args[0]);
        try {
            this.disk.mkdir([...this.cwdParts, ...dp], fn);
            this.disk.flush();
        }
        catch (e) {
            this.term.println(e.message);
        }
    }
    _rd(args) {
        if (!args[0]) {
        ...
        if (ent === null) {
            this.term.println('ディレクトリが見つかりません');
            return;
        }
        if (ent.filter(e => e.name !== '.' && e.name !== '..').length > 0) {
            this.term.println('ディレクトリは空ではありません');
            return;
        }
    ...
    _type(args) {
        if (!args[0]) {
            this.term.println('ファイル名を指定してください');
            return;
        }
        const [dp, fn] = this._splitPath(args[0]);
        const data = this.disk.readFile([...this.cwdParts, ...dp], fn);
        if (!data) {
            this.term.println(`ファイルが見つかりません - ${fn}`);
            return;
        }
        let line = '';
    ...
    _del(args) {
        ...
        if (t === '*.*' || t === '*') {
            ...
            for (const e of ent)
                if (!(e.attr & 0x10)) {
                    this.disk.deleteEntry(this.cwdParts, e.name);
                    cnt++;
                }
            this.disk.flush();
            this.term.println(`${cnt} 個のファイルを削除しました`);
            return;
        }
    ...
    _ren(args) {
        if (args.length < 2) {
            this.term.println('ファイル名を2つ指定してください');
            return;
        }
        const [dp, on] = this._splitPath(args[0]);
        if (this.disk.renameEntry([...this.cwdParts, ...dp], on, args[1].toUpperCase()))
            this.disk.flush();
        else
            this.term.println(`ファイルが見つかりません - ${on}`);
    }
    _move(args) {
        if (args.length < 2) {
            this.term.println('移動元と移動先を指定してください');
            return;
        }
        const [sdp, sfn] = this._splitPath(args[0]);
    ...
    _echo(args) {
        if (!args.length || args[0].toUpperCase() === 'ON' || args[0].toUpperCase() === 'OFF') {
            this.term.println('ECHO is ON');
            return;
        }
    ...
    _set(args) {
        if (!args.length) {
            for (const [k, v] of Object.entries(this.env))
                this.term.println(`${k}=${v}`);
            return;
        }
        const full = args.join(' '), eq = full.indexOf('=');
        if (eq >= 0) {
            const k = full.slice(0, eq).toUpperCase(), v = full.slice(eq + 1);
            if (v)
                this.env[k] = v;
            else
                delete this.env[k];
            ...
            this.term.println(this.env[k] !== undefined ? `${k}=${this.env[k]}` : `環境変数 ${k} は定義されていません`);
        }
    }
    _path(args) {
        if (!args[0]) {
            this.term.println('PATH=' + this.env.PATH);
            return;
        }
    ...
    _promptCmd(args) {
        if (!args[0]) {
            this.term.println('PROMPT=' + this.env.PROMPT);
            return;
        }
    ...
    _find(args) {
        ...
        if (!q) {
            this.term.println('使用法: FIND "文字列" [ファイル名]');
            return;
        }
    ...
    _sort(args) {
        if (!args[0]) {
            this.term.println('使用法: SORT ファイル名');
            return;
        }
        const [dp, fn] = this._splitPath(args[0]);
        const data = this.disk.readFile([...this.cwdParts, ...dp], fn);
        if (!data) {
            this.term.println('ファイルが見つかりません');
            return;
        }
    ...
    _if(args) {
        ...
        if (m) {
            ...
            if (exists) {
                const p = this._splitArgs(m[2]);
                this._dispatch(p[0].toUpperCase(), p.slice(1));
            }
            return;
        }
    ...
    _help(args) {
        ...
        if (args[0]) {
            this.term.println(`${args[0].toUpperCase()} の詳細ヘルプは未実装です`);
            return;
        }
        ...
        try {
            ...
            const tick = () => {
                ...
                while (ran < CYCLES_PER_TICK) {
                    if (cpu.halted)
                        break; // キー待ちor HLT → 次のtickへ
                    cpu.step();
                    ran++;
                    if (performance.now() - t0 > 14)
                        break; // 14ms超えたら中断して次フレーム
                }
                // VRAMレンダリング
                if (window.renderVRAM)
                    window.renderVRAM();
                // 本物のHLTかどうか判定
...
window.MSDos = {
    ...
    saveCom(name, bytes) {
        try {
            this.disk.writeFile([], name, new Uint8Array(bytes));
            this.disk.flush();
            if (window.onDiskChange)
                window.onDiskChange();
            return true;
